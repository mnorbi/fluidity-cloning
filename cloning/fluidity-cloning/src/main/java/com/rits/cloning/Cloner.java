...
public class Cloner
{
	...
	protected Object fastClone(final Object o, final Map<Object, Object> clones, Set<Class<?>> activationGroups) throws IllegalAccessException
	{
		final Class<? extends Object> c = o.getClass();
		final IFastCloner fastCloner = fastCloners.get(c);
		if (fastCloner != null) return fastCloner.clone(o, this, clones, activationGroups);
		return null;
	}
	...
	public <T> T fastCloneOrNewInstance(final Class<T> c, Set<Class<?>> activationGroups)
	{
		try
		{
			final T fastClone = (T) fastClone(c, null, activationGroups);
	...
	public <T> T deepClone(final T o){
		T ret = this.deepClone(o, DefaultActivationGroup.class);
		return ret;
	}
	...
	public <T> T deepClone(final T o, Class<?> ... activationGroups) {
		if (activationGroups == null || activationGroups.length == 0){
			throw new IllegalArgumentException("activation-groups-must-not-be-empty");
		}
		...
		try
		{
			return cloneInternal(o, clones, new LinkedHashSet<Class<?>>(Arrays.asList(activationGroups)));
		} catch (final IllegalAccessException e)
		{
	...
	public <T> T deepCloneDontCloneInstances(final T o, Set<Class<?>> activationGroups, final Object... dontCloneThese)
	{
		if (o == null) return null;
		if (!cloningEnabled) return o;
		if (dumpClonedClasses)
		...
		try
		{
			return cloneInternal(o, clones, activationGroups);
		} catch (final IllegalAccessException e)
		{
			throw new CloningException("error during cloning of " + o, e);
		}
	 ...
	 */
	public <T> T shallowClone(final T o, Set<Class<?>> activationGroups)
	{
		if (o == null) return null;
		if (!cloningEnabled) return o;
		try
		{
			return cloneInternal(o, null, activationGroups);
		} catch (final IllegalAccessException e)
		{
			throw new CloningException("error during cloning of " + o, e);
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <T> T cloneInternal(final T o, final Map<Object, Object> clones, Set<Class<?>> activationGroups) throws IllegalAccessException
	{
		if (o == null) return null;
		...
		final Object fastClone = fastClone(o, clones, activationGroups);
		if (fastClone != null)
		{
			if (clones != null)
			{
		...
		if (clz.isArray())
		{
			...
			for (int i = 0; i < length; i++)
			{
				...
				final Object clone = clones != null ? cloneInternal(v, clones, activationGroups) : v;
		...
		for (final Field field : fields)
		{
			...
			if (!Modifier.isStatic(modifiers))
			{
				if (nullTransient && Modifier.isTransient(modifiers))
				{
					...
					if (!type.isPrimitive())
					{
						setField(field, newInstance, o, null, activationGroups);
					}
				} else
				{
					final Object fieldObject = field.get(o);
					final boolean shouldClone = shouldClone(field, activationGroups);
					final Object fieldObjectClone = clones != null ? (shouldClone ? cloneInternal(fieldObject, clones, activationGroups) : fieldObject) : fieldObject;
					setField(field, newInstance, o, fieldObjectClone, activationGroups);
		...
		finishedCloning(newInstance, o, activationGroups);
		return newInstance;
	}
	...
	public boolean shouldClone(final Field field, Set<Class<?>> activationGroups) {
		boolean ret = (cloneSynthetics || (!cloneSynthetics && !field.isSynthetic())) && (cloneAnonymousParent || ((!cloneAnonymousParent && !isAnonymousParent(field))));
		return ret;
	}
	 ...
	 */
	public <T, E extends T> void copyPropertiesOfInheritedClass(final T src, final E dest, Set<Class<?>> activationGroups)
	{
		...
		for (final Field field : fields)
		{
			if (!Modifier.isStatic(field.getModifiers()))
			{
				try
				{
					...
					{
						setField(field, dest, src, fieldObject, activationGroups);
					}
				} catch (final IllegalArgumentException e)
				{
					throw new RuntimeException(e);
	...
	public void setField(Field field, Object newInstance,
		...
		BasicFieldSetters.DEFAULT.getFieldSetter().setField(field, newInstance, oldInstance, fieldDefaultValue);
	}
}
